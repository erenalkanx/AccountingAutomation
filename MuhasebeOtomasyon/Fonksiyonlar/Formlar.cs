...
namespace MuhasebeOtomasyon.Fonksiyonlar
{
    class Formlar
    {
        ...
        public int StokHareketleri(bool Ac = false)
        {
            //Modul_Stok.frmStokHareketleri frm = new Modul_Stok.frmStokHareketleri();      // eğer bu şekilde çalıştırılırsa ayrı pencerede açılır
            //frm.ShowDialog();


            Modul_Stok.frmStokHareketleri frm = new Modul_Stok.frmStokHareketleri();        //eğer bu şekilde çalıştırılırsa form içinde açılır
            if (Ac)
            {
                frm.Secim = Ac;
                frm.ShowDialog();
        ...
        public int CariHareketleri(bool Ac = false)
        {
        ...
        public void BankaCekCikisi(int Id = 0, bool Ac = false)
        {
        ...
        public int CekListesi(bool Secim = false)
        {
            ...
            {
                foreach (DevExpress.XtraEditors.XtraForm K in AnaForm.ActiveForm.MdiChildren)   //aktif olan tüm alt formları kontrol et
                {
                    if(K.Text == "Çek Listesi") //Çek Listesi textine sahip forma gelince 
                    {
                        K.BringToFront();   //o formu öne çıkar
                        return AnaForm.Aktarma;     
                    }
                }


                frm.MdiParent = AnaForm.ActiveForm;     //şu an açık olan formu, ceklistesi formunun parent'ı yap
                frm.Show(); //ceklistesi formunu aç
            }
            return AnaForm.Aktarma;
        }
